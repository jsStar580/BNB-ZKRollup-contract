...
async function main() {
    ...
    const treasuryRegisterFee = await BNB-ZKRollup.getZNSNamePrice(treasuryName)
    const gasRegisterFee = await BNB-ZKRollup.getZNSNamePrice(gasName)
    const sherRegisterFee = await BNB-ZKRollup.getZNSNamePrice(sherName)
    const gavinRegisterFee = await BNB-ZKRollup.getZNSNamePrice(gavinName)

    console.log('Register ZNS for treasury, gas, sher and gavin...')
    let registerZnsTx = await BNB-ZKRollup.registerZNS(
        treasuryName,
    ...
    registerZnsTx = await BNB-ZKRollup.registerZNS(
        gasName,
        '0x56744Dc80a3a520F0cCABf083AC874a4bf6433F3',
